...
public class AuthPolicy {
    ...
    public static class PolicyDocument {
        ...
        public static PolicyDocument getAllowStarPolicy(String region, String awsAccountId, String restApiId, String stage) {
        	AuthPolicy.PolicyDocument policyDocument = new AuthPolicy.PolicyDocument(region, awsAccountId, restApiId, stage);
        	while (!policyDocument.statements.isEmpty()) {
            	policyDocument.statements.remove(0);
        	}
        	policyDocument.allowStatement = new Statement("Allow", "execute-api:Invoke", Arrays.asList("*"), new HashMap<>());
        	policyDocument.statements.add(policyDocument.allowStatement);
        	return policyDocument;
        }
    }
